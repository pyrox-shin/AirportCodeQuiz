...
python -m PyInstaller --onedir --noconsole --name AviationQuiz --hidden-import=list --hidden-import=main --hidden-import=airlines --hidden-import=theme --hidden-import=records launcher_main.py
if errorlevel 1 (
...
echo ============================================
python -m PyInstaller --onedir --name AviationQuiz_debug --hidden-import=list --hidden-import=main --hidden-import=airlines --hidden-import=theme --hidden-import=records launcher_main.py
if errorlevel 1 (
...
echo ============================================
for %%D in (dist\AviationQuiz dist\AviationQuiz_debug) do (
    if exist worldmap.geojson   xcopy /y worldmap.geojson "%%D\"
    if exist sounds             xcopy /y /e /i sounds "%%D\sounds\"
    if exist airport            xcopy /y /e /i airport "%%D\airport\"
    if exist airlines           xcopy /y /e /i airlines "%%D\airlines\"
)
...
echo  A settings.json file will be created automatically next to the exe on first
echo  run (dark mode + best-score records) - no need to copy one yourself.
echo ============================================
pause
exit /b 0
