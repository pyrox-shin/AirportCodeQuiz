...
class AirportQuiz:
    ...
    遊戲規則沿用目前 V3：
    - 題目由 airport.csv 提供：name, code, lat, lon
    - 輸入自動大寫
    - 正確的三碼會自動進下一題
    - 錯誤的三碼完全不反應
    - Enter 跳過：計入 skipped、播放 wrong.mp3、顯示答案後進下一題
    - 答對播放 correct.mp3
    - 全部完成播放 finish.mp3
    - 進度 = correct + skipped
    - 地圖第一題聚焦目前題目；之後鏡頭聚焦剛完成的航點
    - 地圖使用 GeoJSON，Zoom 4x，Camera 平滑移動
    """
    ...
    def latlon_to_point(self, lat, lon):
        """
        將經緯度轉換為地圖繪製點 (含緯度修正，解決地圖橫向拉長/扁平問題)
        """
        ...
        # 基本像素轉換
        x = (float(lon) + 180.0) / 360.0 * (w * self.map_zoom) * cos_lat
        y = (90.0 - float(lat)) / 180.0 * (h * self.map_zoom)
    ...
    def load_questions(self):
        ...
        try:
            with AIRPORT_FILE.open(
                ...
                if missing:
                    raise ValueError(
                        "airport.csv 必須包含欄位：name, code, lat, lon；"
                        f"目前缺少：{', '.join(sorted(missing))}"
                    )

                rows = []
                for row in reader:
                    # Excel/CSV 欄位可能帶 BOM 或前後空白
                    clean = {
    ...
    def handle_text_input(self, text):
        ...
        # 只接受英文字母；pygame TEXTINPUT 通常已經是文字輸入
    ...
    def submit_answer(self):
        ...
        # 地圖移到剛剛完成的航點
        self.focus_on_airport(self.current, duration=1.2)

        # 讓音效與鏡頭動畫自然進行一點，再進下一題
    ...
    def skip_question(self):
        ...
        self.feedback_color = RED
        self.feedback_until = time.perf_counter() + 0.35
        self.answer = ""
        self.next_question_at = time.perf_counter() + 0.35
    ...
    def map_dimensions(self):
        # 地圖的基準尺寸以目前 viewport 為基礎，保持 4x zoom
        w = max(self.map_rect.width, 1)
        h = max(self.map_rect.height, 1)
        return w * self.map_zoom, h * self.map_zoom

    def focus_on_airport(self, airport, duration=1.2):
    ...
    def animate_map_to(self, dest_x, dest_y, duration=1.2):
        if duration <= 0:
    ...
    def draw_world_map(self, surface):
        ...
        for lon in range(-180, 181, 30):
            ...
            pygame.draw.line(surface, GRID, (sx, 0), (sx, self.map_rect.height), 1)

        for lat in range(-60, 91, 30):
            _, y = self.latlon_to_point(lat, 0)
            sy = int(y + self.map_offset_y)
            pygame.draw.line(surface, GRID, (0, sy), (self.map_rect.width, sy), 1)
        ...
        if self.world_map:
            for feature in self.world_map.get("features", []):
                ...
                for polygon in polygons:
                    points = [
                        (int(x + self.map_offset_x), int(y + self.map_offset_y))
                        for x, y in polygon
                    ]
                    if len(points) >= 3:
                        pygame.draw.polygon(surface, MAP_LAND, points)
                        pygame.draw.lines(surface, MAP_OUTLINE, True, points, 1)
    ...
    def layout(self):
        ...
        self.header_rect = pygame.Rect(margin, 10, w - margin * 2, 58)
        self.stats_rect = pygame.Rect(margin, 68, w - margin * 2, 48)

        map_y = 124
        map_h = max(300, int(h * 0.42))  # 地圖高度佔比大幅提升
        self.map_rect = pygame.Rect(margin, map_y, w - margin * 2, map_h)

        self.progress_rect = pygame.Rect(
            margin, self.map_rect.bottom + 8,
            w - margin * 2, 6
        )

        card_y = self.progress_rect.bottom + 8
    ...
    def build_result_layout(self):
        margin = max(24, int(self.screen.get_width() * 0.035))
        card = self.card_rect
    ...
    def draw_result_page(self, surface):
        ...
        y = result_label.get_height() + self.result_left_rect.y + 25
        for line in stats:
    ...
    def max_result_scroll(self):
        # Estimate based on actual line count and current viewport.
        y = 0
    ...
    def finish_game(self):
        ...
        self.finished = True
        self.processing_answer = False
        self.finish_elapsed = self.elapsed_time()
    ...
    def handle_event(self, event):
        ...
        if self.finished:
            if event.type == pygame.MOUSEWHEEL:
                ...
                if self.result_view_rect.collidepoint(event.pos):
